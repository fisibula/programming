...
class Student
{
  //We define the class Student with a variety of data members, and two member function prototypes, for now we will place everithing in the public access region: 
...
{
  // The member function definition for void Student::Initialize() takes a variety of parameters:
    firstName = fn; 
    lastName = ln; 
    this->middleInitial = mi;  // optional use of this
    this->gpa = gpa;  // required, explicit use of this. Here disambiguating gpa with this on the left hand side of the assigment indicates to set the data member, which is pointed to by this , to the value of the input parameter, gpa.
...
// }
// Each data member's name is preceded with this. We are accessing the data member of an object that is pointed to be this. this is the implicit first input parameter to this function, and is a constant pointer to the object that invoked this function. 
